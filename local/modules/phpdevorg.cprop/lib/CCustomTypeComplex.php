...
class CCustomTypeComplex extends StringType
{
    ...
    public static function GetUserTypeDescription(): array
    {
        return array(
            'PROPERTY_TYPE' => 'S',
            'USER_TYPE_ID' => 'complexfield',
//            'USER_TYPE' => 'complexfield',
    ...
    public static function GetEditFormHTML(array $userField, ?array $additionalParameters): string
    {
        ...
        if (!empty($userField['SETTINGS'])) {
            ...
            return '<span>' . Loc::getMessage('IEX_CPROP_ERROR_INCORRECT_SETTINGS') . '</span>';//Не заполнен список полей в настройках свойства
        }
        ...
        foreach ($arFields as $code => $arItem) {
            if ($arItem['TYPE'] === 'string') {
                $result .= self::showString($code, $arItem['TITLE'], $userField, $additionalParameters);
            } else if ($arItem['TYPE'] === 'file') {
                $result .= self::showFile($code, $arItem['TITLE'], $userField, $additionalParameters);
            } else if ($arItem['TYPE'] === 'text') {
                $result .= self::showTextarea($code, $arItem['TITLE'], $userField, $additionalParameters);
            } else if ($arItem['TYPE'] === 'date') {
                $result .= self::showDate($code, $arItem['TITLE'], $userField, $additionalParameters);
            } else if ($arItem['TYPE'] === 'element') {
                $result .= self::showBindElement($code, $arItem['TITLE'], $userField, $additionalParameters);
            }
        }
    ...
    {
        $btnAdd = Loc::getMessage('IEX_CPROP_SETTING_BTN_ADD'); //Добавить
        $settingsTitle = Loc::getMessage('IEX_CPROP_SETTINGS_TITLE'); //Список полей
...
//        return $result;
//    }
    static function PrepareSettings($userField): array
    {
        $result = [];
        if (!empty($userField['SETTINGS'])) {
            foreach ($userField['SETTINGS'] as $code => $value) {
                $result[$code] = $value;
            }
        }
    ...
    private static function showString($code, $title, $userField, $additionalParameters)
    {
        $result = '';

        $v = !empty($additionalParameters['VALUE'][$code]) ? $additionalParameters['VALUE'][$code] : '';
        $result .= '<tr>
                    <td align="right">' . $title . ': </td>
                    <td>
                        <input type="text" name="'.$additionalParameters["NAME"].'['.$code.']" value="'.$v.'" />
                    </td>
                </tr>';
    ...
    private static function getOptionList($selected = 'string')
    {
        ...
        $arOption = [
            ...
            'htmleditor' => Loc::getMessage('IEX_CPROP_FIELD_TYPE_HTML_EDITOR'),
        ];
    ...
    public static function onAfterFetch($arProperty, $arValue): array|string
    {
